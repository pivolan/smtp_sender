...
func SendTls(host string, port string, username string, password string, proxyUrl string, timeout time.Duration, from string, to string, subject string, bodyHtml string, filename string, fileBody []byte) (err error) {
	c, err := SmtpTlsAuth(host, port, username, password, proxyUrl, timeout)
	if err != nil {
		return
	}
	err = c.Mail(from)
...
func SendPlain(host string, port string, username string, password string, proxyUrl string, timeout time.Duration, from string, to string, subject string, bodyHtml string, filename string, fileBody []byte) (err error) {
	c, err := SmtpPlainAuth(host, port, username, password, proxyUrl, timeout)
	if err != nil {
		return
	}
	err = c.Mail(from)
